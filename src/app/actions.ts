...
const formSchema = z.object({
  ...
  componentCode: z
    .string()
    .min(10, { message: 'Component code must be at least 10 characters long.' }),
...
export async function analyzeComponentAction(
  ...
  const { componentName, componentCode, idToken } = validatedFields.data;
  let user = null;
  ...
  try {
    const analysis = await analyzeComponent({ componentName, componentCode });
